...
export class SignupComponent {
  ...
  signup(){
    this.signupForm.patchValue({ location: this.signupForm.get('location')?.value});

    this.userService.signUpUser(this.signupForm).subscribe(
      (data: User) => {
        this.temp.userId = data.userId;
        this.temp.location = data.location;
        this.addLocation();
